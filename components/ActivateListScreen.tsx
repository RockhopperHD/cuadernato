...
interface ActivateListScreenProps {
  onActivate: (list: string[], checksum: string, name: string, password?: string, showVulgar?: boolean) => void;
  onBack: () => void;
  dictionary: DictionaryEntry[];
}

export const ActivateListScreen: React.FC<ActivateListScreenProps> = ({ onActivate, onBack, dictionary }) => {
  ...
  const handleDictionaryImport = (code: string) => {
    ...
    onActivate(normalizedIds, checksum, 'Imported List', undefined, true); // No password for old format
  }
  ...
  const handleCustomImport = (code: string) => {
    try {
        ...
        onActivate(idsAsArray, idSum.toString(), listData.name, listData.password, listData.showVulgar);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen w-full p-4 bg-[#191724] text-slate-200">
      <div className="w-full max-w-2xl text-center">
        ...
        <h1 className="text-4xl font-bold mb-2">Activate List</h1>
        <p className="text-slate-400 mb-8">
          Paste a list code to activate words in the dictionary. This will become your active list.
        </p>
        <textarea
          value={listCode}
        ...
        <button
          onClick={handleAttemptActivation}

...
export const ListBuilder: React.FC<ListBuilderProps> = ({ setMode, dictionary, onActivate }) => {
    ...
    return (
        <div className="flex flex-col h-screen bg-[#191724] text-slate-200 font-sans p-4 sm:p-6 lg:p-8">
            {renderModal()}
            <header className="flex items-center justify-between w-full max-w-5xl mx-auto mb-4">
                <div className="flex items-center gap-4">
                     <button onClick={() => setMode('title')} className="p-2 rounded-full hover:bg-slate-700 transition-colors">
                        <BackIcon className="w-6 h-6"/>
                    </button>
                    <div>
                        <h1 className="text-4xl font-bold">List Builder</h1>
                        <p className="text-slate-400">Create, import, and activate custom word lists</p>
                    </div>
                </div>
                <div className="relative">
                    <button onClick={() => setDoneMenuOpen(p => !p)} className="bg-green-600 text-white font-bold py-2 px-4 rounded-xl hover:bg-green-700 transition-colors text-md flex items-center gap-2">
                        <CheckIcon className="w-5 h-5" /> Done
                    </button>
                    {doneMenuOpen && (
                        <div className="absolute top-full right-0 mt-2 w-48 bg-slate-700 border border-slate-600 rounded-md shadow-lg z-20 p-1">
                            <button onClick={handleActivate} className="w-full text-left p-2 rounded hover:bg-slate-600">Activate This List</button>
                            <button onClick={handleExport} className="w-full text-left p-2 rounded hover:bg-slate-600">Export to Code</button>
                            <button onClick={() => setMode('title')} className="w-full text-left p-2 rounded hover:bg-slate-600">Back to Title</button>
                        </div>
                    )}
                </div>
            </header>
            ...
            <main className="flex-grow w-full max-w-5xl mx-auto flex flex-col gap-4 overflow-hidden">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <input type="text" value={listName} onChange={e => setListName(e.target.value)} placeholder="List Name..." className="bg-black/50 text-white placeholder-slate-400 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                    <input type="password" value={listPassword} onChange={e => setListPassword(e.target.value)} placeholder="List Password (optional)..." className="bg-black/50 text-white placeholder-slate-400 p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500" />
                    <label className="flex items-center justify-center gap-2 cursor-pointer bg-black/50 p-3 rounded-xl">
                        <span className="font-medium text-slate-300">Show Vulgar Words</span>
                        <div className="relative"><input type="checkbox" className="sr-only" checked={listShowVulgar} onChange={() => setListShowVulgar(p => !p)} /><div className={`block w-10 h-6 rounded-full ${listShowVulgar ? 'bg-indigo-600' : 'bg-slate-600'}`}></div><div className={`dot absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${listShowVulgar ? 'transform translate-x-4' : ''}`}></div></div>
                  </label>
                </div>

                <div className="flex items-center gap-4 bg-black/30 p-2 rounded-xl">
                    <button onClick={() => setModal('stats')} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white font-semibold py-2 px-4 rounded-lg transition-colors">{wordPairs.length} Words</button>
                    <button onClick={() => setModal('import')} className="flex-1 bg-slate-700 hover:bg-slate-600 text-white font-semibold py-2 px-4 rounded-lg transition-colors">Import List...</button>
                    <button onClick={() => setModal('clearConfirm')} disabled={wordPairs.length === 0} className="flex-1 bg-red-800/80 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors disabled:bg-slate-800 disabled:text-slate-500 disabled:cursor-not-allowed flex items-center justify-center gap-2"><TrashIcon className="w-5 h-5"/> Clear List</button>
                </div>
                
                <div className="flex-grow bg-black/30 p-4 rounded-xl overflow-y-auto space-y-2">
                    {wordPairs.length === 0 ? (
                        <div className="h-full flex items-center justify-center"><p className="text-slate-400">Your list is empty. Add word pairs below.</p></div>
                    ) : (
                        wordPairs.map(p => (
                            <div key={p.id} className="grid grid-cols-12 gap-2 items-center animate-fade-in">
                                ...
                                {p.ambiguousMeanings ? (
                                    <select value={p.selectedMeaningAsIn} onChange={(e) => handleMeaningChange(p.id, e.target.value)} className="col-span-5 bg-sky-400 text-black font-semibold p-3 rounded-lg text-lg truncate focus:outline-none focus:ring-2 focus:ring-sky-200">
                                ...
                                <div className="col-span-2 flex justify-center items-center gap-2">
                                    ...
                                    <button onClick={() => removePair(p.id)} className="text-red-300 hover:text-red-500 transition-colors p-1"><CloseIcon className="w-6 h-6" strokeWidth="2.5" /></button>
                                </div>
                            </div>
                        ))
                    )}
